...
export const AudioPlayer = forwardRef<HTMLAudioElement, AudioPlayerProps>(
  ({ src, label, className, onEnded, autoPlay }, ref) => {
    ...
    // EQ state: bass, mid, treble (-12 to +12 dB)
    ...
    // Initialize Web Audio graph once per audio element
    useEffect(() => {
      ...
      <div className={cn('space-y-3', className)}>
        ...
        {showEQ && (
          <div className="rounded-lg border border-border bg-secondary/30 p-3 space-y-2.5">
            ...
            {[
              { label: 'Bass', value: bass, set: setBass, freq: '250 Hz' },
              { label: 'Mid', value: mid, set: setMid, freq: '1 kHz' },
              { label: 'Treble', value: treble, set: setTreble, freq: '4 kHz' },
            ].map(band => (
