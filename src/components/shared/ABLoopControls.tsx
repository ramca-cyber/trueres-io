...
export function ABLoopControls({ audioElement, className }: ABLoopControlsProps) {
  ...
  return (
    <div className={cn('flex items-center gap-1.5', className)}>
      ...
      >
        A{hasA ? ` ${formatTimePrecise(pointA!)}` : ''}
      </Button>
      ...
      >
        B{hasB ? ` ${formatTimePrecise(pointB!)}` : ''}
      </Button>
      ...
      {hasA && hasB && (
        <Button
          ...
          onClick={toggle}
          aria-label={active ? 'Disable A-B loop' : 'Enable A-B loop'}
        >
          {active ? '⟳ On' : '⟳ Off'}
        </Button>
      )}
      ...
      {(hasA || hasB) && (
        <Button variant="ghost" size="sm" className="h-7 px-2 text-xs text-muted-foreground"
          onClick={clear} aria-label="Clear A-B loop">
          ✕
        </Button>
      )}
    </div>
